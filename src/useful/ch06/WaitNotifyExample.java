...
public class WaitNotifyExample {
	...
	static class Producer extends Thread {
		...
		public void run() {
			...
			synchronized (sharedResource) {
				...
				// sharedResource 를 쓸 수 있도록 다른 작업자들에게 알려 주기 - notify
	...
	static class Consumer extends Thread {
		...
		public void run() {
			synchronized (sharedResource) {
				...
				System.out.println("데이터가 생산 완료 되어서 사용 가능함");
	...
	public static void main(String[] args) {
		...
		// 소비자 스레드 시작
		consumer.start(); // 소비자는 --> run() --> 동기화 블록 --> wait(스레드 대기)		
		try {
			Thread.sleep(100);
		} catch (InterruptedException e) {
			e.printStackTrace();
		...
		// run() --> for() --> list add () --> notify() 알려주고 있다.
